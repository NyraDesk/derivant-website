...
Con app local-first come NotezAI, non devi scegliere tra funzionalità moderne (come AI) e privacy. Puoi avere entrambe.

Prova NotezAI gratis: derivant.ai
...
const BlogArticlePage = () => {
  ...
  const formatContent = (text: string) => {
    return text.split('\n\n').map((paragraph, index) => {
      ...
      if (paragraph.trim().length > 0 && paragraph.trim().length < 100 && !paragraph.includes('-') && !paragraph.match(/^\d\./)) {
        return (
          <h2 key={index} className="text-3xl lg:text-4xl font-bold text-white mb-8 leading-tight mt-16">
            {paragraph.trim()}
          </h2>
        );
      }
      ...
      if (paragraph.includes('\n-') || paragraph.includes('\n•')) {
        ...
        return (
          <div key={index} className="mb-8">
            {title && <p className="text-slate-300 text-lg leading-relaxed mb-4">{title}</p>}
            <ul className="list-none space-y-3 ml-4">
              {items.map((item, i) => (
                <li key={i} className="text-slate-300 text-lg leading-relaxed flex items-start">
      ...
      if (paragraph.match(/^\d\./m)) {
        ...
        return (
          <div key={index} className="mb-8">
            {lines.map((line, i) => (
              <p key={i} className="text-slate-300 text-lg leading-relaxed mb-3">
                {line}
              </p>
            ))}
          </div>
      ...
      // Regular paragraph
      return (
        <p key={index} className="text-slate-300 text-lg leading-relaxed mb-8">
          {paragraph}
        </p>
      );
    });
        ...
        <div className="max-w-5xl mx-auto px-6 lg:px-8">
          ...
          <div className="animate-fade-in-up animation-delay-400">
            ...
            <div className="mt-16 p-6 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl">
              <p className="text-white/70 text-sm mb-2">Scritto da</p>
              <p className="text-white text-lg font-medium">{article.author}, Founder di NotezAI</p>
            </div>
